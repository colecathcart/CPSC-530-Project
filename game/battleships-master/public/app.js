...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startMultiPlayer() {
    ...
    socket.on('fire', id => {
      ...
      if(!(cpuDestroyerCount === 0 || cpuDestroyerCount === 10 )){
        inprogress = true
      }
      if(!(cpuSubmarineCount === 0 || cpuSubmarineCount === 10 )){
        inprogress = true
      }
      if(!(cpuCruiserCount === 0 || cpuCruiserCount === 10 )){
        inprogress = true
      }
      if(!(cpuBattleshipCount === 0 || cpuBattleshipCount === 10 )){
        inprogress = true
      }
      if(!(cpuCarrierCount === 0 || cpuCarrierCount === 10 )){
        inprogress = true
      }
      const square = userSquares[id]
      square.classList.add(square.dataset.id)
      if(inprogress){
        square.classList.add('inprogress')
      }
